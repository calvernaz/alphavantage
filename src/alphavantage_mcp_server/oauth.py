...
class OAuthConfig:
    ...
    def __post_init__(self):
        ...
        elif self.token_validation_method == "introspection" and not self.introspection_endpoint:
            raise ValueError("introspection_endpoint required for introspection validation")
...
class OAuthResourceServer:
    ...
    def __init__(self, config: OAuthConfig):
        ...
        logger.info(f"Initialized OAuth resource server for {config.resource_server_uri}")
    ...
    async def handle_resource_metadata_request(self, request: Request) -> JSONResponse:
        ...
        try:
            ...
            return JSONResponse(
                content={"error": "server_error", "error_description": "Failed to generate metadata"},
                status_code=500
            )
    ...
    async def validate_jwt_token(self, token: str) -> TokenValidationResult:
        ...
        try:
            ...
            logger.warning(f"Token validation failed: Invalid audience. Expected: {self.config.resource_server_uri}")
    ...
    async def validate_token_introspection(self, token: str) -> TokenValidationResult:
        ...
        if not self.config.introspection_endpoint:
            return TokenValidationResult(False, error="Introspection endpoint not configured")
            ...
            if token_audience:
                if isinstance(token_audience, list):
                    if self.config.resource_server_uri not in token_audience:
                        logger.warning(f"Token audience mismatch. Expected: {self.config.resource_server_uri}, Got: {token_audience}")
                        return TokenValidationResult(False, error="Invalid audience")
                elif token_audience != self.config.resource_server_uri:
                    logger.warning(f"Token audience mismatch. Expected: {self.config.resource_server_uri}, Got: {token_audience}")
                    return TokenValidationResult(False, error="Invalid audience")
            
            logger.debug(f"Token introspection successful for subject: {introspection_result.get('sub')}")
    ...
    async def authenticate_request(self, request: Request, session_id: str = None) -> Tuple[bool, Optional[TokenValidationResult]]:
        """
    ...
    def create_www_authenticate_header(self) -> str:
        ...
        metadata_url = urljoin(self.config.resource_server_uri, self.config.resource_metadata_path)
        return f'Bearer resource="{metadata_url}"'
    
    async def create_unauthorized_response(self, error: str = "invalid_token", 
    ...
    async def create_forbidden_response(self, error: str = "insufficient_scope",
                                      description: str = None) -> Response:
...
def create_oauth_config_from_env() -> Optional[OAuthConfig]:
    ...
    return OAuthConfig(
        ...
        session_binding_enabled=os.getenv("OAUTH_SESSION_BINDING_ENABLED", "true").lower() == "true"
    )
