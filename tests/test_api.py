...
from alphavantage_mcp_server.api import fetch_earnings_calendar, fetch_earnings_call_transcript, fetch_sma
...
async def test_fetch_sma():
    ...
    assert tech_analysis_key is not None, "Response should contain Technical Analysis section for SMA"
    ...
    assert "3: Last Refreshed" in meta_data, "Meta Data should contain last refreshed date"
...
async def test_fetch_sma_with_response_limiting():
    ...
    sma_data = result[tech_analysis_key]
    assert len(sma_data) <= 10, f"Response should be limited to 10 data points, got {len(sma_data)}"
    ...
    if summary_key in result:
        ...
        assert "total_data_points_available" in summary, "Summary should show total available data points"
        assert "data_points_returned" in summary, "Summary should show returned data points"
        assert "note" in summary, "Summary should contain explanation note"
        assert summary["data_points_returned"] == len(sma_data), "Summary count should match actual data"
...
async def test_fetch_sma_large_response_handling():
    ...
    assert tech_analysis_key is not None, "Response should contain Technical Analysis section"
    
    # Check that response respects the default limit
    sma_data = result[tech_analysis_key]
    assert len(sma_data) <= 100, f"Response should be limited to 100 data points by default, got {len(sma_data)}"
