...
async def handle_mcp_request(request_data: Dict[str, Any], oauth_server: OAuthResourceServer = None) -> Dict[str, Any]:
    """
...
async def handle_tools_list_request(request_id: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_jsonrpc_error(request_id, -32603, f"Failed to list tools: {str(e)}")


async def handle_tools_call_request(request_id: Any, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_jsonrpc_error(request_id, -32603, f"Tool execution failed: {str(e)}")
...
async def handle_prompts_list_request(request_id: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_jsonrpc_error(request_id, -32603, f"Failed to list prompts: {str(e)}")


async def handle_prompts_get_request(request_id: Any, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_jsonrpc_error(request_id, -32603, f"Failed to get prompt: {str(e)}")


def validate_oauth_request(event: Dict[str, Any], oauth_server: OAuthResourceServer) -> Dict[str, Any]:
    ...
    try:
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            return {
                'authenticated': False,
                'response': create_oauth_error_response(401, "invalid_token", "Missing or invalid authorization header")
            }
        ...
        return {
            'authenticated': False,
            'response': create_oauth_error_response(500, "server_error", f"OAuth validation error: {str(e)}")
        }
...
def create_oauth_error_response(status_code: int, error_type: str, description: str) -> Dict[str, Any]:
